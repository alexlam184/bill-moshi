    ...
    <div className="mx-auto min-h-dvh w-full max-w-[1180px] bg-white lg:grid lg:grid-cols-[250px_1fr] lg:border-x lg:border-line lg:shadow-2xl">
      <aside className="hidden border-r border-line bg-white px-5 py-7 lg:flex lg:flex-col">
        ...
        <div className="mt-7 border-t border-line pt-5">
          <div className="mb-2 flex items-center justify-between px-2"><p className="text-[0.68rem] font-extrabold uppercase tracking-[0.14em] text-muted">Groups</p><Link href="/groups/new" aria-label="Create group" className="grid size-7 place-items-center rounded-lg text-brand-dark hover:bg-brand-soft"><Plus size={15} /></Link></div>
          <div className="grid gap-1">
            <Link href="/myself" onClick={selectPersonal} className={`flex min-h-10 items-center gap-2.5 rounded-xl px-2.5 text-sm font-bold ${menuPersonalSelected ? "bg-brand-soft text-brand-dark" : "text-muted hover:bg-slate-50"}`}><UserRound size={17} /><span className="min-w-0 flex-1 truncate">Myself</span>{menuPersonalSelected && <Check size={15} />}</Link>
            <Link href="/debts" onClick={() => selectGroup(undefined)} className={`flex min-h-10 items-center gap-2.5 rounded-xl px-2.5 text-sm font-bold ${debtSelected ? "bg-brand-soft text-brand-dark" : "text-muted hover:bg-slate-50"}`}><HandCoins size={17} /><span className="min-w-0 flex-1 truncate">Debt records</span>{debtSelected && <Check size={15} />}</Link>
            {snapshot.groups.map((group) => <Link key={group.id} href={`/groups/${group.id}`} onClick={() => selectGroup(group.id)} className={`flex min-h-10 items-center gap-2.5 rounded-xl px-2.5 text-sm font-bold ${menuSelectedGroupId === group.id ? "bg-slate-100 text-ink" : "text-muted hover:bg-slate-50"}`}><span className="text-lg">{group.emoji}</span><span className="min-w-0 flex-1 truncate">{group.name}</span>{menuSelectedGroupId === group.id && <Check size={15} className="text-brand-dark" />}</Link>)}
          </div>
        </div>

        <Link href={addExpenseHref} className="mt-6 flex min-h-12 items-center justify-center gap-2 rounded-xl bg-brand px-4 text-sm font-extrabold text-[#103a55] transition hover:bg-[#62afe5]"><Plus size={19} /> Add record</Link>
        <div className="mt-auto rounded-xl border border-line p-3 text-xs leading-5 text-muted"><span className={`mr-2 inline-block size-2 rounded-full ${isOnline ? "bg-success" : "bg-warning"}`} />{isOnline ? "Online" : "Offline"}{pendingCount > 0 && <span className="block font-semibold text-warning">{pendingCount} waiting to sync</span>}</div>
      </aside>

      <div className="min-w-0">
        {!focusedFlow && <header className="sticky top-0 z-30 grid h-16 grid-cols-[44px_1fr_70px] items-center border-b border-line bg-white/95 px-3 backdrop-blur lg:hidden">
          <button type="button" onClick={() => setMenuOpen(true)} className="grid size-10 place-items-center rounded-xl text-ink hover:bg-slate-100" aria-label="Open group menu"><Menu size={23} /></button>
          <Link href={personalSelected ? "/records/mine" : selectedGroup ? `/groups/${selectedGroup.id}` : "/"} className="min-w-0 px-2 text-center"><span className="block truncate text-sm font-extrabold tracking-tight">{personalSelected ? "👤 Myself" : selectedGroup ? `${selectedGroup.emoji} ${selectedGroup.name}` : "Bill Moshi"}</span><span className="block text-[0.62rem] font-bold uppercase tracking-[0.12em] text-muted">{personalSelected ? "Personal expenses" : selectedGroup ? "Current group" : "All groups"}</span></Link>
          <span className={`justify-self-end rounded-full px-2 py-1 text-[0.62rem] font-extrabold ${isOnline ? pendingCount ? "bg-warning-soft text-warning" : "bg-success-soft text-success" : "bg-warning-soft text-warning"}`}>{!isOnline ? "Offline" : pendingCount ? `${pendingCount} pending` : "Synced"}</span>
        </header>}

        <main className={expenseFlow || debtFlow ? "mx-auto min-h-dvh w-full p-0 md:max-w-[820px] md:px-8 md:py-8" : calendarFlow ? "mx-auto min-h-dvh w-full p-0 md:max-w-[980px] md:px-8 md:py-8" : "mx-auto min-h-[calc(100dvh-4rem)] w-full max-w-[820px] px-4 pb-28 pt-6 sm:px-7 md:min-h-dvh md:px-10 md:pb-12 md:pt-10"}>{children}</main>
        ...
        {!focusedFlow && (debtNavigation ? <nav className="safe-bottom fixed inset-x-0 bottom-0 z-40 mx-auto grid max-w-[1180px] grid-cols-5 border-t border-line bg-white/95 px-2 pt-2 backdrop-blur lg:hidden" aria-label="Debt records navigation">
          ...
          <Link href="/debts/new" aria-label="Add debt record" className="mx-auto -mt-5 grid size-14 place-items-center rounded-2xl border-4 border-white bg-brand text-[#103a55] shadow-lg transition active:scale-95"><Plus size={27} strokeWidth={2.6} /></Link>
          ...
          <Link href={addExpenseHref} aria-label="Add record" className="mx-auto -mt-5 grid size-14 place-items-center rounded-2xl border-4 border-white bg-brand text-[#103a55] shadow-lg transition active:scale-95"><Plus size={27} strokeWidth={2.6} /></Link>
...
function GroupDrawer({ selectedGroupId, personalSelected, onClose }: { selectedGroupId?: string; personalSelected: boolean; onClose(): void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 lg:hidden" role="dialog" aria-modal="true" aria-label="Select a group">
      <button type="button" className="absolute inset-0 bg-slate-950/35 backdrop-blur-[2px]" onClick={onClose} aria-label="Close group menu" />
      <aside className="safe-bottom relative flex h-full w-[86%] max-w-[340px] flex-col bg-white px-4 py-5 shadow-2xl animate-rise">
        <div className="flex items-center justify-between px-1"><Brand /><button type="button" onClick={onClose} className="grid size-10 place-items-center rounded-xl text-muted hover:bg-slate-100" aria-label="Close group menu"><X size={21} /></button></div>
        ...
        <Link href="/" onClick={() => { selectGroup(undefined); onClose(); }} className="mt-2 flex min-h-11 items-center justify-center text-sm font-bold text-muted">Manage all groups</Link>
        <div className="mt-auto rounded-xl bg-slate-50 p-3 text-xs text-muted"><span className={`mr-2 inline-block size-2 rounded-full ${isOnline ? "bg-success" : "bg-warning"}`} />{isOnline ? "Online" : "Offline"}{pendingCount > 0 ? ` · ${pendingCount} waiting to sync` : " · Up to date"}</div>
      </aside>
    </div>
  );
}
...
function MobileLink({ href, label, active, icon }: { href: string; label: string; active: boolean; icon: ReactNode }) {
  return <Link href={href} className={`grid min-h-14 place-items-center content-center gap-1 text-[0.66rem] font-bold ${active ? "text-brand-dark" : "text-muted"}`}><span className={active ? "text-brand-dark" : "text-slate-400"}>{icon}</span>{label}</Link>;
}
