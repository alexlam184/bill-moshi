...
export function GroupMembersScreen({ groupId }: { groupId: string }) {
  ...
  function confirmInvitationAction() {
    if (confirmAction === "regenerate") regenerate();
    if (confirmAction === "disable" && activeInvitation) {
      revokeInvitation(activeInvitation.id);
      setConfirmAction(undefined);
    }
  }
  ...
  return (
    <div className="grid gap-6">
      ...
      {isOwner && <Card className="overflow-hidden"><div className="flex items-center gap-3 border-b border-line p-5"><span className="grid size-11 place-items-center rounded-xl bg-brand-soft text-brand-dark"><Link2 size={21} /></span><div><h2 className="font-extrabold">Group Invitation Link</h2><p className="mt-0.5 text-xs text-muted">Approval required is on by default.</p></div></div><div className="grid gap-4 p-5">{activeInvitation ? <><div className="flex min-w-0 items-center gap-2 rounded-xl bg-slate-50 p-2 pl-3"><p className="min-w-0 flex-1 truncate text-xs font-semibold text-muted">{inviteUrl || "Preparing link…"}</p><Button type="button" onClick={copyLink} className="min-h-9 px-3">{copied ? <Check size={16} /> : <Copy size={16} />}{copied ? "Copied" : "Copy"}</Button><span className="sr-only" aria-live="polite">{copied ? "Invitation link copied" : ""}</span></div><div className="flex flex-wrap gap-2"><span className="rounded-full bg-warning-soft px-3 py-1.5 text-xs font-bold text-warning"><ShieldCheck className="mr-1 inline" size={14} /> Approval required</span><span className="rounded-full bg-slate-100 px-3 py-1.5 text-xs font-bold text-muted">Default role: {activeInvitation.defaultRole}</span></div><div className="grid grid-cols-2 gap-3"><Button type="button" variant="secondary" onClick={() => setConfirmAction("regenerate")}><RefreshCw size={16} /> Regenerate</Button><Button type="button" variant="danger" onClick={() => setConfirmAction("disable")}>Disable Link</Button></div></> : <Button type="button" onClick={() => createInvitation(groupId)}><UserPlus size={17} /> Create Group Invitation</Button>}</div></Card>}

      {isOwner && <section><div className="mb-3 flex items-center justify-between"><h2 className="text-lg font-extrabold">Pending Requests</h2>{pending.length > 0 && <span className="rounded-full bg-warning-soft px-2.5 py-1 text-xs font-extrabold text-warning">{pending.length} waiting</span>}</div><Card className="divide-y divide-line overflow-hidden">{pending.length === 0 ? <div className="p-6 text-center text-sm text-muted">No one is waiting for approval.</div> : pending.map((request) => <div key={request.id} className="virtual-list-item p-4"><div className="flex items-center gap-3"><Avatar name={request.requesterName} color="#27AE60" /><div className="min-w-0 flex-1"><p className="truncate text-sm font-bold">{request.requesterName}</p><p className="truncate text-xs text-muted">{request.requesterEmail}</p></div><p className="text-[0.68rem] text-muted">{new Intl.DateTimeFormat("en-CA", { month: "short", day: "numeric" }).format(new Date(request.requestedAt))}</p></div><div className="mt-3 grid grid-cols-2 gap-2 pl-[3.25rem]"><Button type="button" variant="secondary" onClick={() => reviewJoinRequest(request.id, "rejected")}><X size={16} /> Reject</Button><Button type="button" onClick={() => reviewJoinRequest(request.id, "approved", "member")}><Check size={16} /> Approve</Button></div></div>)}</Card></section>}
      ...
      {confirmAction && <InvitationConfirmModal action={confirmAction} onClose={() => setConfirmAction(undefined)} onConfirm={confirmInvitationAction} />}
    </div>
  );
}
