...
export function EventsScreen() {
  ...
  return (
    <div className="grid gap-6">
      <PageTitle eyebrow="All groups" title="Events" subtitle="Specific trips, dinners, and activities, organized by group." action={<Link href="/events/new" className="grid size-11 place-items-center rounded-xl bg-brand text-brand-ink" aria-label="Create event"><CalendarPlus size={20} /></Link>} />
      <div className="relative"><Search className="pointer-events-none absolute left-3.5 top-3.5 text-slate-400" size={18} /><input aria-label="Search events" name="event-search" autoComplete="off" value={query} onChange={(event) => setQuery(event.target.value)} className={`${fieldClass} pl-11`} placeholder="Search Events or Groups…" /></div>
      <div className="grid gap-3 sm:grid-cols-2">
        {events.map((event) => {
          ...
          const balance = me ? eventNetBalances(event.id, snapshot.members, snapshot.records, snapshot.settlements).get(me.id) ?? 0 : 0;
          return (
            <Link key={event.id} href={`/events/${event.id}`} className="virtual-list-item group rounded-[1.3rem] border border-line bg-white p-5 card-shadow transition hover:-translate-y-0.5 hover:border-brand">
              ...
              <div className="mt-5 flex items-end justify-between border-t border-line pt-4"><div><p className="text-[0.68rem] font-bold uppercase tracking-wide text-muted">Total spent</p><p className="mt-1 text-sm font-extrabold">{formatMoney(totalEventSpending(event.id, snapshot.records), event.baseCurrency)}</p></div><p className={`text-sm font-extrabold ${balance >= 0 ? "text-success" : "text-danger"}`}>{balance > 0 ? `+${formatMoney(balance, event.baseCurrency)}` : balance < 0 ? `−${formatMoney(Math.abs(balance), event.baseCurrency)}` : "Settled"}</p></div>
            </Link>
          );
        })}
      </div>
