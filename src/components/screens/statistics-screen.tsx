...
export function StatisticsScreen({ eventId }: { eventId: string }) {
  ...
  const records = snapshot.records.filter((record) => record.recordType === "expense" && record.eventId === eventId);
  const total = totalEventSpending(eventId, snapshot.records);
  const categoryTotals = snapshot.categories.map((category) => ({ category, total: records.filter((record) => record.categoryId === category.id).reduce((sum, record) => sum + record.amountBase, 0) })).filter((item) => item.total > 0).sort((a, b) => b.total - a.total);
  const dates = records.map((record) => record.transactionDate.slice(0, 10));
