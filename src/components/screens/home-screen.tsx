...
import { AlertTriangle, ArrowDownLeft, ArrowUpRight, CalendarDays, ChevronRight, Plus, ReceiptText, UsersRound, X } from "lucide-react";
...
export function HomeScreen() {
  const { snapshot, selectedGroupId, personalContext, selectGroup, dismissGroupDeletionNotice } = useBillMoshi();
  ...
  for (const event of snapshot.events) {
    ...
    const balance = eventNetBalances(event.id, snapshot.members, snapshot.records, snapshot.settlements).get(currentMember.id) ?? 0;
  ...
  for (const group of snapshot.groups) {
    ...
    if (!currentMember) continue;
    const currencies = new Set(snapshot.records
      .filter((record) => record.groupId === group.id && !record.eventId)
      .map((record) => record.baseCurrency));
    for (const currency of currencies) {
      const balance = groupDailyNetBalances(group.id, currency, snapshot.groupMembers, snapshot.records).get(currentMember.id) ?? 0;
  ...
  const reportingSummary = overallReportingBalanceForUser(snapshot.currentUser.id, reportingCurrency, snapshot.members, snapshot.groupMembers, snapshot.records, snapshot.settlements);
  const missingRates = recordsMissingReportingRate(snapshot.records, reportingCurrency);
  ...
  return (
    <div className="grid min-w-0 gap-7">
      ...
      {snapshot.groupDeletionNotices.map((notice) => <Card key={notice.groupId} className="flex items-start gap-3 border-warning/40 bg-warning-soft p-4"><span className="grid size-10 shrink-0 place-items-center rounded-xl bg-white text-warning"><AlertTriangle size={19} /></span><div className="min-w-0 flex-1"><p className="font-extrabold">“{notice.groupName}” is no longer available</p><p className="mt-1 text-xs leading-5 text-muted">The owner deleted the Group or removed your access. Its local events and records were removed from this device.</p><p className="mt-1 text-[0.68rem] text-muted">Detected {new Intl.DateTimeFormat("en-CA", { month: "short", day: "numeric", year: "numeric", hour: "numeric", minute: "2-digit" }).format(new Date(notice.detectedAt))}</p></div><button type="button" onClick={() => dismissGroupDeletionNotice(notice.groupId)} className="grid size-10 shrink-0 place-items-center rounded-xl bg-white/80 text-muted hover:text-ink" aria-label={`Dismiss ${notice.groupName} deletion notice`}><X size={18} /></button></Card>)}
      ...
      <Card className="overflow-hidden border-0 bg-gradient-to-br from-balance-start via-balance-middle to-white p-5 sm:p-6">
        ...
        <Link href="/records/missing-rates" className={`mt-4 flex min-h-11 items-center justify-between rounded-xl px-3.5 text-sm font-extrabold ${missingRates.length > 0 ? "bg-warning-soft text-warning" : "bg-white/80 text-brand-dark"}`}><span className="min-w-0 truncate whitespace-nowrap">{missingRates.length > 0 ? `${missingRates.length} older ${missingRates.length === 1 ? "record needs" : "records need"} a rate` : "Rates complete"}</span><ChevronRight size={17} className="shrink-0" /></Link>
      </Card>
      ...
      <section>
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          {snapshot.groups.slice(0, 4).map((group) => {
            ...
            const dailyCount = snapshot.records.filter((record) => record.groupId === group.id && !record.eventId).length;
            ...
            const myBalances = groupCurrencyBalancesForUser(group.id, snapshot.currentUser.id, snapshot.events, snapshot.members, snapshot.groupMembers, snapshot.records, snapshot.settlements);
      ...
      <section>
        ...
        <Card className="divide-y divide-line overflow-hidden">
          {recentRecords.length === 0 ? <div className="p-7 text-center text-sm text-muted">No records yet.</div> : recentRecords.map((record) => {
            ...
            const payer = snapshot.members.find((item) => item.id === record.payerId) ?? snapshot.groupMembers.find((item) => item.id === record.payerId) ?? (record.payerId === snapshot.currentUser.id ? snapshot.currentUser : undefined);
            const verb = record.recordType === "expense" ? "paid" : record.recordType === "income" ? "received" : "sent";
            return <Link href={`/records/${record.id}`} key={record.id} className="flex items-center gap-3 p-4 transition hover:bg-slate-50"><span className={`grid size-10 place-items-center rounded-xl text-xl ${record.recordType === "income" ? "bg-success-soft" : record.recordType === "transfer" ? "bg-brand-soft" : "bg-slate-50"}`}>{category?.emoji ?? "🧾"}</span><div className="min-w-0 flex-1"><p className="truncate text-sm font-bold">{record.description}</p><p className="mt-0.5 truncate text-xs text-muted">{!record.groupId ? "Myself · Personal" : event?.name ?? `${group?.name ?? "Group"} · Daily`} · {payer?.name} {verb}</p></div><div className="text-right"><p className={`text-sm font-extrabold ${record.recordType === "income" ? "text-success" : record.recordType === "transfer" ? "text-brand-dark" : ""}`}>{record.recordType === "income" ? "+" : ""}{formatMoney(record.amountOriginal, record.currencyOriginal)}</p><p className={`mt-0.5 text-[0.68rem] font-bold ${record.syncStatus === "synced" ? "text-success" : "text-warning"}`}>{record.syncStatus === "synced" ? "Synced" : "Pending"}</p></div></Link>;
          })}
        </Card>
      </section>

      <div className="flex items-center justify-center gap-2 rounded-xl bg-slate-50 px-4 py-3 text-xs text-muted"><ReceiptText size={15} /> Personal records stay yours in Google Sheets and Drive.</div>
    </div>
  );
}
