...
export function MembersScreen({ eventId }: { eventId: string }) {
  ...
  return (
    <div className="grid gap-6">
      ...
      <PageTitle eyebrow={`${members.length} participants`} title="Event members" subtitle={`Members are inherited from ${group?.name ?? "the parent group"}.`} action={group && <Link href={`/groups/${group.id}/members`} className="grid size-11 place-items-center rounded-xl bg-brand text-brand-ink" aria-label="Manage group members"><UsersRound size={20} /></Link>} />
