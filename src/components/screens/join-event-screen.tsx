...
export function JoinGroupScreen({ token }: { token: string }) {
  ...
  if (!hydrated && !valid) return <main className="grid min-h-dvh place-items-center bg-app px-4"><div className="size-12 animate-pulse rounded-2xl bg-brand-soft" /></main>;
  if (!valid) return <main className="grid min-h-dvh place-items-center bg-app px-4"><Card className="w-full max-w-md p-8 text-center"><Brand /><span className="mx-auto mt-8 grid size-16 place-items-center rounded-2xl bg-danger-soft text-danger"><LockKeyhole size={27} /></span><h1 className="mt-5 text-2xl font-extrabold tracking-tight">This invitation isn’t available</h1><p className="mt-2 text-sm leading-6 text-muted">The group owner may have disabled it, or the link may have expired.</p><Link href="/" className="mt-6 inline-flex min-h-11 items-center rounded-xl bg-brand px-5 text-sm font-bold text-brand-ink">Open Bill Moshi</Link></Card></main>;
