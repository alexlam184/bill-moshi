...
import { ArrowDownLeft, ArrowUpRight, ChevronRight, Plus, ReceiptText, UserRound, WalletCards } from "lucide-react";
...
export function MyselfOverviewScreen() {
  ...
  const records = snapshot.expenses
  ...
  const amountInDefaultCurrency = (record: Expense) => {
  ...
  const expenses = records.filter((record) => record.recordType === "expense").reduce((sum, record) => sum + amountInDefaultCurrency(record), 0);
  ...
  return (
    <div className="grid gap-6 animate-rise">
      <PageTitle eyebrow="Personal overview" title="Myself" subtitle="Private income and expenses that do not belong to any group." action={<Link href="/expenses/new?personal=1" className="grid size-11 place-items-center rounded-xl bg-brand text-[#103a55]" aria-label="Add personal record"><Plus size={20} /></Link>} />

      <section className="rounded-[1.6rem] bg-gradient-to-br from-[#dff2ff] via-[#eff8fe] to-white p-5 sm:p-7">
      ...
      <section className="grid grid-cols-2 gap-3 sm:grid-cols-4" aria-label="Personal record totals">
        ...
        <SummaryCard label="Expense" value={formatMoney(expenses, currency)} tone="danger" />
      ...
      <section>
        <div className="mb-3 flex items-center justify-between gap-3"><div><h2 className="text-lg font-extrabold tracking-tight">Recent records</h2><p className="mt-0.5 text-xs text-muted">Newest personal records first</p></div><Link href="/records/mine" className="flex items-center gap-1 text-sm font-extrabold text-brand-dark">View records <ChevronRight size={16} /></Link></div>
        <Card className="divide-y divide-line overflow-hidden">
          {recentRecords.length === 0 ? <EmptyState icon={<WalletCards size={24} />} title="No personal records" body="Add an income or expense that belongs only to you." action={<Link href="/expenses/new?personal=1" className="inline-flex min-h-11 items-center gap-2 rounded-xl bg-brand px-4 text-sm font-extrabold text-[#103a55]"><Plus size={17} /> Add record</Link>} /> : recentRecords.map((record) => {
            const category = snapshot.categories.find((item) => item.id === record.categoryId);
            return <Link key={record.id} href={`/expenses/${record.id}`} className="flex items-center gap-3 p-4 transition hover:bg-slate-50"><span className={`grid size-11 shrink-0 place-items-center rounded-xl text-xl ${record.recordType === "income" ? "bg-success-soft" : "bg-slate-50"}`}>{category?.emoji ?? "🧾"}</span><div className="min-w-0 flex-1"><p className="truncate text-sm font-extrabold">{record.description}</p><p className="mt-1 text-xs text-muted">{new Intl.DateTimeFormat("en-CA", { month: "short", day: "numeric", year: "numeric" }).format(new Date(record.transactionDate))} · {record.syncStatus === "synced" ? "Synced" : "Pending"}</p></div><div className="text-right"><p className={`text-sm font-extrabold ${record.recordType === "income" ? "text-success" : ""}`}>{record.recordType === "income" ? "+" : ""}{formatMoney(record.amountOriginal, record.currencyOriginal)}</p><ChevronRight size={17} className="ml-auto mt-1 text-slate-300" /></div></Link>;
          })}
        </Card>
      </section>
