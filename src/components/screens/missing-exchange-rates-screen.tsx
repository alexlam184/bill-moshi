...
export function MissingExchangeRatesScreen() {
  ...
  return <div className="grid gap-6">
    ...
    {missing.length === 0 ? <Card><EmptyState icon={<CheckCircle2 size={24} />} title="All records are covered" body={`Every record can be included in your ${reportingCurrency} All Groups balance.`} /></Card> : <div className="grid gap-3">{missing.map((expense) => {
      ...
      return <Card key={expense.id} className="virtual-list-item p-4 sm:p-5"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><Link href={`/expenses/${expense.id}`} className="truncate text-sm font-extrabold hover:text-brand-dark">{expense.description}</Link><p className="mt-1 truncate text-xs text-muted">{context} · {formatRateRecordDate(expense.transactionDate)}</p></div><p className="shrink-0 text-sm font-extrabold">{formatMoney(expense.amountBase, expense.baseCurrency)}</p></div><form onSubmit={(event) => save(event, expense.id)} className="mt-4 grid gap-2" noValidate><label className="text-xs font-bold text-muted" htmlFor={`rate-${expense.id}`}>1 {expense.baseCurrency} equals</label><div className="grid grid-cols-[1fr_auto_auto] gap-2"><input id={`rate-${expense.id}`} name={`rate-${expense.id}`} autoComplete="off" aria-label={`Manual ${expense.baseCurrency} to ${reportingCurrency} rate for ${expense.description}`} type="number" min="0" step="any" inputMode="decimal" value={values[expense.id] ?? ""} onChange={(event) => setValues((current) => ({ ...current, [expense.id]: event.target.value }))} placeholder="e.g. 1.38…" className="min-w-0 rounded-xl border border-line bg-white px-3.5 text-right text-sm font-extrabold outline-none focus:border-brand focus:ring-4 focus:ring-brand-soft" /><span className="grid min-h-11 place-items-center rounded-xl bg-slate-50 px-3 text-xs font-extrabold text-muted">{reportingCurrency}</span><Button type="submit" className="px-3" aria-label={`Save manual rate for ${expense.description}`}><Save size={16} /></Button></div>{Number(values[expense.id]) > 0 && <p className="text-right text-xs font-bold text-success">{formatMoney(expense.amountBase, expense.baseCurrency)} → {formatMoney(expense.amountBase * Number(values[expense.id]), reportingCurrency)}</p>}{errors[expense.id] && <p role="alert" aria-live="assertive" className="text-xs font-bold text-danger">{errors[expense.id]}</p>}</form></Card>;
    })}</div>}
  </div>;
}
