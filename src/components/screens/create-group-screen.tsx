...
export function CreateGroupScreen() {
  ...
  const dirty = Boolean(name || description || emoji !== "👨‍👩‍👧‍👦" || currency !== snapshot.currentUser.defaultCurrency);
  ...
  function submit(event: FormEvent) {
    ...
    if (!name.trim()) {
      ...
      requestAnimationFrame(() => document.querySelector<HTMLElement>("[name='group-name']")?.focus());
      return;
    }
      ...
      <form onSubmit={submit} className="grid gap-5" noValidate>
        <Card className="grid gap-5 p-5 sm:p-6">
          ...
          <Field label="Group name" error={error}>
            <input className={fieldClass} placeholder="e.g. Moshi Family…" value={name} onChange={(event) => { setName(event.target.value); setError(""); }} />
          </Field>
          ...
          <Field label="Description" hint="Optional — a short reminder of who this group is for.">
            <textarea className={`${fieldClass} min-h-24 py-3`} placeholder="e.g. Family trips and shared activities…" value={description} onChange={(event) => setDescription(event.target.value)} />
          </Field>
        </Card>
        <Card className="grid gap-5 p-5 sm:p-6">
